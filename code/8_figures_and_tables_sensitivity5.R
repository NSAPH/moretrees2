...
for (i in 1:length(dataset)) { # datasets
   ds <- dataset[i]
   for (mod in 3:nmods) { # models
      
      # Read in results of moretrees model 
      load(file = paste0("./results/mod", mod, "_split0_", ds, "_sens2.Rdata"))
      approx_ml[i, 1, mod - 1] <- moretrees_results$mod$hyperparams$ELBO
      mod0 <- moretrees_results
      load(file = paste0("./results/mod", mod, "_split25_", ds, "_sens2.Rdata"))
      ...
      tabfile <- paste0("./figures/mod", mod, "_", ds, "_table_sens2.tex")
...
ml_plot <- ggplot(approx_ml2) + 
   ...
   scale_shape_discrete(name = expression(PM[2.5]*" break"),
                        ...
                        solid = F)
pdf(file = paste0("./figures/vb_bound_sens2.pdf"), width = 6, height = 4)
ml_plot
dev.off()
