...
for (i in 1:length(dataset)) { # datasets
  ...
  if (i == 1) {
    pdf(file = "./figures/cvd_nested_plot_results.pdf", height = 16, width = 15)
    beta_indiv_plot_fun(pltdat, tr, xlab = xlab, lab.widths = lab.widths,
                        lab.txt.width = lab.txt.width, axis.height = 1.4,
                        cil_min = -8, ciu_max = 8,
                        lab.txt.size = 4, digits = 0, 
                        axis.txt.size = 10,
                        plot_depth = 4, wrap_labs = FALSE,
                        force_lims = T,
                        errorbar.height = 0.3, point.size = 2)
    dev.off()
  }
  if (i == 2) {
    pdf(file = "./figures/resp_nested_plot_results.pdf", height = 11, width = 15)
    beta_indiv_plot_fun(pltdat, tr, xlab = xlab, lab.widths = lab.widths,
                        lab.txt.width = lab.txt.width, axis.height = 1.1,
                        cil_min = -8, ciu_max = 8,
