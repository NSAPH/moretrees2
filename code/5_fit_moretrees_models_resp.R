...
dt[ , c("id", "adate", "ssa_state_cd", "pm25_lag01_case", "pm25_lag01_control") := NULL]
...
mod1 <- moretrees::moretrees(X = as.matrix(dt[, "pm25"]), 
                             W = NULL,
                             y = rep(1, nrow(dt)),
...
# Save
save(moretrees_results, file = "./results/mod1_split0_northEast_resp.RData")

# Model 2: linear covariate control ------------------------------------------------------------------------------------
mod2 <- moretrees::moretrees(X = as.matrix(dt[, "pm25"]), 
...
# save
save(moretrees_results, sd_tmmx, sd_rmax, file = "./results/mod2_split0_northEast_resp.RData")
...
mod3 <- moretrees::moretrees(X = as.matrix(dt[, "pm25"]), 
...
# save
save(moretrees_results, sd_tmmx, sd_rmax, file = "./results/mod3_split0_northEast_resp.RData")
